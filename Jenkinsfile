pipeline {
    ...
    environment {
        ...
        REMOTE_HOST = "13.55.127.58" // 원격(spring) 서버IP(Public IP)
        REMOTE_DIR = "/home/ec2-user/deploy" // 원격 서버에 파일 복사할 경로 
        SSH_CREDENTIALS_ID = "8550605c-78b6-4c88-bdf4-451d3cf3279f" // Jenkins SSH 자격      증명 ID
    }
    ...
    stages {
        ...
        stage('Prepare Jar') {
            steps { // 빌드된 소스 jar 파일을 지정한 이름(app.jar) 으로 카피함. 지정한 이름은 위 environment에 있음
